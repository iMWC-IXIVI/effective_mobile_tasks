...
from database.crud import get_hash_params_dynamics, get_hash_params_results
...
@pytest_asyncio.fixture(scope='function')
async def mock_redis_cache_results_hit(mocker: MockerFixture):
    """Redis get возвращает не None results"""
    ...
    params = [
        ...
        },
        {
            'oil_id': 'AST_20',
            'delivery_type_id': 'Street_55',
            'delivery_basis_id': 'st.70',
            'limit': 150
        }
    ]
    hash_params = [await get_hash_params_results(**param) for param in params]
    cache_name = 'trading:results'

    def dynamic_effect(key: str):
        """Динамичное возвращение значение мока"""

        if key == f'{cache_name}:{hash_params[0]}':
            return redis_data_results_10
        elif key == f'{cache_name}:{hash_params[1]}':
            return redis_data_results_1
        elif key == f'{cache_name}:{hash_params[2]}':
            return '[]'
